...
identify securities. Required for trade settlement and regulatory filings.

```
NVDA CUSIP: 67066G104
```

Fetched via two sources in priority order (`src/sec_client.py → get_cusip_from_ticker()`):
1. OpenFIGI API (free tier — sometimes includes the `cusip` field)
2. SEC XBRL DEI facts — `dei:SecurityCUSIP` tag (fallback; not all companies report this)

Non-critical enrichment data. Pipeline continues with `None` if unavailable.
...
A 12-character alphanumeric code that uniquely identifies a financial instrument globally.
Open standard maintained by Bloomberg. The "composite FIGI" covers all exchanges where
an instrument trades.

```
NVDA composite FIGI: BBG000BBJQV0
```

Fetched via OpenFIGI API (`src/sec_client.py → get_figi_from_ticker()`).
Stored in `result["figi"]` and appears in CSV/XML output. Non-critical enrichment data.
